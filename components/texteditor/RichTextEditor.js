...
const theme = {
  ...
  // Video
  video: {
    container: 'my-4 block w-full',
    left: 'text-left',
    center: 'text-center',
    right: 'text-right',
  },
  // Video node container
  'video-node-container': 'block w-full my-4 [&>div]:!block',
};
...
// 클라이언트 전용 래퍼 컴포넌트
function LexicalEditorComponent({ contents, setContents }) {
      ...
      <LexicalComposer initialConfig={initialConfig}>
        ...
        <div className="editor-inner relative">
          <RichTextPlugin
            contentEditable={
              <ContentEditable
                className="editor-input min-h-[400px] p-4 focus:outline-none whitespace-pre-wrap break-words overflow-wrap-anywhere"
...
export default function RichTextEditor({ contents, setContents }) {
  ...
  if (!isMounted) {
    return (
      <div className="editor-container max-w-4xl mx-auto mt-8 border rounded-lg shadow-lg">
        ...
        <div className="min-h-[400px] p-4 bg-white relative">
          <div className="text-gray-400">에디터를 로딩 중...</div>
        </div>
      </div>
    );
  }
  ...
  // 클라이언트에서만 실제 에디터 렌더링
  return <LexicalEditorComponent contents={contents} setContents={setContents} />;
} 